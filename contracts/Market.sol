...
contract Market is IMarket, ReentrancyGuard {
    ...
    function initialize(uint256 _indexPrice, address _clearAnchor, uint256 _clearAnchorRatio, address _maker, uint8 _marketType, address _tradeToken) external {
    ...
    function priceToOpen(uint256 id, uint256 price, uint256 indexPrice, uint256 indexPriceTimestamp) external nonReentrant onlyRouter {
        ...
        //mint tradeToken
        (bool _success, bytes memory data) = tradeToken.call(abi.encodeWithSelector(0x40c10f19, order.taker, order.takerFee));
        require(_success && (data.length == 0 || abi.decode(data, (bool))), 'Market:priceToOpen mint tradeToken fail');
    }
